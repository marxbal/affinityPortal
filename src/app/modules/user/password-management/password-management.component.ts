...
import { LOGGED_IN } from 'src/app/constants/local.storage';
  ...
  updatePassword() {
    if (this.password == "" || this.cpassword == "") {
      ...
      if (this.validatePassword(this.password) !== "proceed") {
        ...
        this.spinner.show();
        this.authLogin.callNonAuth("/api-non-auth/updateUser", {
          userInfo: this.userHolder
        }).subscribe(
          result => {
            this.changePasswordRequest.status = "0";
            let param = {
              ChangePassRequest: this.changePasswordRequest
            }
