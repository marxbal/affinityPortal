...
export class MotorIssuanceService {
  ...
  mapRetrieveQuote(affinity: Affinity, result) {
    ...
    this.commonService.getCoverageByPolicy("P", this.motorAff.quotationNumber, this.motorAff.motorDetails.motorTypeId).subscribe(
      (result) => {
  ...
  mapRetrievePolicy(affinity: Affinity, result) {
    ...
    this.commonService.getCoverageByPolicy("A", this.motorAff.policyNumber, this.motorAff.motorDetails.motorTypeId).subscribe(
      (result) => {
        let totalLossDamagePrem = 0;
